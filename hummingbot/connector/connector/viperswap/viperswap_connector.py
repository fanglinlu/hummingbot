...
from hummingbot.connector.connector.viperswap.viperswap_in_flight_order import ViperswapInFlightOrder
...
class ViperswapConnector(ConnectorBase):
    """
    ViperswapConnector connects with viperswap gateway APIs and provides pricing, user account tracking and trading
    functionality.
    """
    ...
    async def initiate_pool(self) -> str:
        ...
        """
        try:
            self.logger().info(f"Initializing Viperswap connector and paths for {self._trading_pairs} pairs.")
            resp = await self._api_request("get", "eth/viperswap/start",
    ...
    async def auto_approve(self):
        """
        Automatically approves Viperswap contract as a spender for token in trading pairs.
        ...
        for token, amount in self._allowances.items():
            if amount <= s_decimal_0:
                amount_approved = await self.approve_viperswap_spender(token)
    ...
    async def approve_viperswap_spender(self, token_symbol: str) -> Decimal:
        """
        Approves Viperswap contract as a spender for a token.
        ...
        if amount_approved > 0:
            self.logger().info(f"Approved Viperswap spender contract for {token_symbol}.")
        else:
            self.logger().info(f"Viperswap spender contract approval failed on {token_symbol}.")
        return amount_approved

    async def get_allowances(self) -> Dict[str, Decimal]:
        """
        Retrieves allowances for token in trading_pairs
        :return: A dictionary of token and its allowance (how much Viperswap can spend).
        """
        ret_val = {}
        ...
        try:
            ...
            if any(item not in resp.keys() for item in required_items):
                ...
                if price is not None and len(exceptions) == 0:
                    ...
                    fee_overrides_config_map["viperswap_maker_fee_amount"].value = Decimal(str(gas_cost))
                    fee_overrides_config_map["viperswap_taker_fee_amount"].value = Decimal(str(gas_cost))
    ...
    async def _create_order(self,
        ...
        try:
            order_result = await self._api_request("post", "eth/viperswap/trade", api_params)
            ...
            self.logger().network(
                f"Error submitting {trade_type.name} order to Viperswap for "
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = ViperswapInFlightOrder(
    ...
    @property
    def in_flight_orders(self) -> Dict[str, ViperswapInFlightOrder]:
